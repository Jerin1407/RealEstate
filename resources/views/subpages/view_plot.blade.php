...
@section('content')
    <section class="px-6 py-10 bg-gray-50" x-data="{ selectedImage: '{{ asset('uploads/' . ($plot->images->where('is_cover', 1)->first()->filename ?? 'images/no-image.jpg')) }}' }">
        <div class="max-w-7xl mx-auto grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">{{ $plot->property_title }}</h2>

                    <h4 class="text-lg font-semibold text-gray-700 mb-2">Property Description</h4>
                    <p class="text-gray-700 leading-relaxed mb-4">{{ strip_tags($plot->property_description) }}</p>
                    ...
                    <p class="font-semibold text-gray-800">Contact: <span class="font-normal">{{ $plot->contact_name }}</span>
                        </p>

                    <p class="text-gray-700 mb-2">
                        <span class="font-semibold">Contact Number:</span> {{ $plot->contact_number }}
                    </p>

                    <div class="flex justify-between items-center mt-8">
                        <p class="text-xl font-bold text-red-600">
                            {{ $plot->locality->locality_name ?? 'N/A' }}
                        </p>
                    </div>
                </div>
            </div>
