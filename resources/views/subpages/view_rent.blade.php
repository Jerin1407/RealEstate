...
@section('title', 'Rent Property Details | Real Estate Thrissur')

@section('content')
    <section class="px-6 py-10 bg-gray-50" x-data="{ selectedImage: '{{ asset('uploads/property/' . ($rent->images->where('is_cover', 1)->first()->filename ?? 'images/no-image.jpg')) }}' }">
        ...
        <div class="max-w-7xl mx-auto grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <!-- Property Code -->
                    <h3 class="text-red-600 font-bold text-lg mb-2">{{ $rent->property_code }}</h3>
                    ...
                    <p class="text-gray-700 mb-2"><span class="font-semibold">Contact:</span> For professional assistance
                        and better deals contact <span class="font-semibold">{{ $rent->contact_name }}</span>
                    </p>
                    ...
                    <div class="flex justify-between items-center mt-8">
                        <p class="text-xl font-bold">
                            Rs. <span class="text-red-600">{{ $rent->price }}</span>
                        </p>
                        <p class="text-xl font-bold text-red-600">{{ $rent->locality->locality_name ?? 'N/A' }}</p>
                    </div>

                    <!-- Contact -->
                    <div class="mt-6 space-y-2">
                        <p class="font-semibold text-gray-800">Contact: <span
                                class="font-normal">{{ $rent->contact_name }}</span>
                        </p>
                        <p class="font-semibold text-gray-800">Contact Number:
                            <span class="font-normal">{{ $rent->contact_number }}</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 lg:h-2/3 ">
                ...
                <form class="space-y-4" action="{{ route('propertyEnquiry') }}" method="POST">
                    ...
                    <input type="hidden" name="property_code" value="{{ $rent->property_code }}">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $rent->property_code }}" readonly
