...
@section('title', 'Flat Property Details | Real Estate Thrissur')

@section('content')
    <section class="px-6 py-10 bg-gray-50" x-data="{ selectedImage: '{{ asset('uploads/property/' . ($flat->images->where('is_cover', 1)->first()->filename ?? 'images/no-image.jpg')) }}' }">
        <div class="max-w-7xl mx-auto grid lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 lg:h-2/3 ">
                ...
                <form class="space-y-4" action="{{ route('propertyEnquiry') }}" method="POST">
                    ...
                    <!-- Hidden Fields -->
                    <input type="hidden" name="property_id" value="{{ $flat->property_id }}">
                    <input type="hidden" name="property_code" value="{{ $flat->property_code }}">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $flat->property_code }}" readonly
